...
public class ControlEdit extends AppCompatActivity {

	private android.widget.EditText rut, name, dateTime, bloodPresure, heartFreq, saturation, breathFreq, temperture, hgt, notes;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView( com.example.controls.R.layout.activity_control_edit);
		rut = (android.widget.EditText) findViewById(com.example.controls.R.id.rut);
		name = (android.widget.EditText) findViewById(com.example.controls.R.id.fullName);
	...
	public void onBackPressed() {
		if (backPressedTime + 3000 > System.currentTimeMillis()) {
			backToast.cancel();
			startActivity(new android.content.Intent(ControlEdit.this, ControList.class));
			super.onBackPressed();
			return;
	...
	private void getControls() {
		String gotRut = getIntent().getExtras().get("rut").toString().trim();
		rut.setText(gotRut);

		String gotName = getIntent().getExtras().get( "name" ).toString();
		name.setText( gotName );

		String gotDateTime = getIntent().getExtras().get("dateTime").toString().trim();
		dateTime.setText(gotDateTime);

		String gotBloodPresure = getIntent().getExtras().get("bloodPresure").toString().trim();
		bloodPresure.setText(gotBloodPresure);

		String gotHeartFreq = getIntent().getExtras().get("heartFreq").toString().trim();
		heartFreq.setText(gotHeartFreq);

		String gotSaturation = getIntent().getExtras().get("saturation").toString().trim();
		saturation.setText(gotSaturation);

		String gotBreathFreq = getIntent().getExtras().get("breathFreq").toString().trim();
		breathFreq.setText(gotBreathFreq);

		String gotTemperture = getIntent().getExtras().get("temperture").toString().trim();
		temperture.setText(gotTemperture);

		String gotHgt = getIntent().getExtras().get("hgt").toString().trim();
		hgt.setText(gotHgt);

		String gotNotes = getIntent().getExtras().get("notes").toString().trim();
	...
	public boolean onCreateOptionsMenu(android.view.Menu menu) {
		...
		getMenuInflater().inflate(com.example.controls.R.menu.menu_control_edit_delete, menu);
	...
	public boolean onOptionsItemSelected(@androidx.annotation.NonNull android.view.MenuItem item) {
		...
		switch (item.getItemId()){
			...
			case com.example.controls.R.id.icon_home:
				...
				startActivity(new android.content.Intent(ControlEdit.this, Welcome.class));
				finish();
				break;

			case com.example.controls.R.id.icon_search:
				startActivity(new android.content.Intent(ControlEdit.this, com.example.controls.View.ControlSearch.class));
				finish();
				break;

			case com.example.controls.R.id.icon_delete:
				vibrating();
				deleteControl();
				finish();
				break;
	...
	private void deleteControl() {
		...
		String gotUid = getIntent().getExtras().get( "uid" ).toString();
		control.setUid(gotUid.toString());
		databaseReference.child( "controls" ).child( control.getUid() ).removeValue();
		android.widget.Toast.makeText(ControlEdit.this, "El Control Se Ha Eliminado Exitosamente!", android.widget.Toast.LENGTH_LONG).show();
		startActivity(new android.content.Intent(ControlEdit.this, ControList.class));
		finish();
	}
...
//        declaring editTexts
		...
		String taken_fullName = name.getText().toString().trim();
...
//       Validating empty fiels
		...
		if (taken_fullName.isEmpty()){
			vibrating();
			name.setError("Debe Ingresar el Nombre del Paciente");
			name.requestFocus();
			return;
		}
...
//        filling Database with Persistance
		...
		android.widget.Toast.makeText(ControlEdit.this, "El Control Se Ha Guardado Exitosamente!", android.widget.Toast.LENGTH_LONG).show();
		startActivity(new android.content.Intent(ControlEdit.this, ControList.class));
		finish();
	}
}